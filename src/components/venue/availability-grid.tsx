...
export function AvailabilityGrid({ venueId, courts }: AvailabilityGridProps) {
  ...
  }, [dateStr, courts, supabase, venueId])
                    ...
                    return (
                      <td key={hour} className="p-1 border-r border-border/50 last:border-0">
                        {isPast ? (
                          ...
                          <button disabled aria-label="Ocupado" className="h-10 w-full bg-red-500/10 border border-red-500/20 text-red-500 rounded flex items-center justify-center text-xs font-medium cursor-not-allowed">
                            Ocupado
                          </button>
                        ) : (
                          <button
                            onClick={() => handleSlotClick(court.id, hour)} aria-label={`Reservar turno de las ${hour} horas`}
