...
export function VenueProfileForm({ venue }: { venue: import("@/types/domain").Venue }) {
...
export function VenueLocationForm({ venue }: { venue: import("@/types/domain").Venue }) {
  const [loading, setLoading] = useState(false)
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      await updateVenueProfile(new FormData(e.currentTarget))
      toast({ title: "Ubicación actualizada correctamente" })
    } catch (error: unknown) {
      toast({ title: "Error", description: error instanceof Error ? error.message : "Error desconocido", variant: "destructive" })
    } finally {
      setLoading(false)
...
export function VenuePaymentSettingsForm({ venue }: { venue: import("@/types/domain").Venue }) {
  const [loading, setLoading] = useState(false)
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      toast({ title: "Configuración de pagos actualizada correctamente" })
    } catch (error: unknown) {
      toast({ title: "Error", description: error instanceof Error ? error.message : "Error desconocido", variant: "destructive" })
    } finally {
      setLoading(false)
