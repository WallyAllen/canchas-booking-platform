...
type SortKey = 'booking_date' | 'courts.name' | 'profiles.full_name' | 'status' | 'total_price'
              ...
              <table className="w-full text-sm text-left">
                <thead className="text-xs text-muted-foreground uppercase bg-muted/50 border-b">
                  <tr>
                    ...
                    <th className="px-6 py-4 font-medium cursor-pointer hover:bg-muted" onClick={() => handleSort('status')}>
                    ...
                    <tr key={booking.id} className="hover:bg-muted/30 transition-colors">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <Badge variant="outline" className={
                          booking.status === 'cancelled' ? 'border-red-500/20 text-red-600 dark:text-red-400 bg-red-500/10 font-bold' : 
                          booking.status === 'confirmed' ? 'border-green-500/20 text-green-600 dark:text-green-400 bg-green-500/10 font-bold' : 
                          'border-blue-500/20 text-blue-600 dark:text-blue-400 bg-blue-500/10 font-bold'
                        }>
                          {booking.status === 'confirmed' ? 'Confirmada' : 
                           booking.status === 'cancelled' ? 'Cancelada' : 
                           booking.status === 'completed' ? 'Completada' : 'No Show'}
                        </Badge>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
